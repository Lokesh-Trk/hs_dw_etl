...
def start_etl(load_id,data_source_cd):
	...
	#	Insert new data created in the load date range
		sql=f"SELECT hospital_key,hospital_cd from {target_database_nm}.dim_hospital where source_cd='{data_source_cd}'"
		cursor.execute(sql)
		...
		for h, t in product(*ranges):
			...
			if not Log.check_status(load_id,etl,insert_fact_table_data[t]["source_table"]+"-"+hospital_data[h][1]+".insert",table_name,"Completed",data_source_cd):
			#if file has been processed for the given load id successfully, then, skip it
				sub_log_id,data_start_ts,data_end_ts = Log.insert_log(load_id,etl,insert_fact_table_data[t]["source_table"]+"-"+hospital_data[h][1]+".insert",table_name,"Started",None,None,None,data_source_cd)
