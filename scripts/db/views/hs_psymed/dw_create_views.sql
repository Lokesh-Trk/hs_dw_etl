DROP VIEW IF EXISTS healthscore_dw.psymed_bill_items_master_view;
CREATE VIEW healthscore_dw.psymed_bill_items_master_view
as select dbi.hospital_key, bill_item_type,bill_item_category_cd,bill_item_category_nm,bill_item_category_desc,bill_item_cd,bill_item_nm,bill_item_amt,transaction_type_cd,
...
where hospital_cd in ('PSYMED')
;
 
DROP VIEW IF EXISTS healthscore_dw.psymed_hospital_master_view;
CREATE VIEW healthscore_dw.psymed_hospital_master_view
...
DROP VIEW IF EXISTS healthscore_dw.psymed_patient_master_view;
CREATE VIEW healthscore_dw.psymed_patient_master_view as
...
where mph.hospital_cd in ('PSYMED');

DROP VIEW IF EXISTS healthscore_dw.psymed_patient_medications_view;
CREATE VIEW healthscore_dw.psymed_patient_medications_view as
...
join  healthscore_dw.psymed_patient_master_view pm
on fpm.patient_key = pm.patient_key
join healthscore_dw.psymed_hospital_master_view hm
on fpm.prescribed_hospital_key = hm.hospital_key;

DROP VIEW IF EXISTS healthscore_dw.psymed_patient_assessments_view;
CREATE VIEW healthscore_dw.psymed_patient_assessments_view as
...
DROP VIEW IF EXISTS healthscore_dw.psymed_hospital_staff_master_view;
CREATE VIEW healthscore_dw.psymed_hospital_staff_master_view as
...
from 
  ...
  where hospital_cd in ('PSYMED');
   ...
   DROP VIEW IF EXISTS healthscore_dw.psymed_patient_visit_view;
CREATE VIEW healthscore_dw.psymed_patient_visit_view as
...
DROP VIEW IF EXISTS healthscore_dw.psymed_patient_careplan_instructions_view;
CREATE VIEW healthscore_dw.psymed_patient_careplan_instructions_view AS
...
JOIN healthscore_dw.psymed_hospital_master_view hm ON fcp.visit_hospital_key = hm.hospital_key
...
as select dbi.hospital_key, bill_item_key,bill_item_type,bill_item_category_cd,bill_item_category_nm,bill_item_category_desc,bill_item_cd,bill_item_nm,bill_item_amt,transaction_type_cd,effective_from_ts,effective_to_ts,rate_category_nm,dbi.active_flg
 from healthscore_dw.dim_bill_items dbi
join healthscore_dw.psymed_hospital_master_view dh
on dbi.hospital_key = dh.hospital_key
;

DROP VIEW IF EXISTS healthscore_dw.psymed_vist_bill_items_view;
CREATE VIEW healthscore_dw.psymed_vist_bill_items_view AS
...
JOIN healthscore_dw.psymed_patient_visit_view pvv ON pvv.patient_visit_key = vbv.patient_visit_key
JOIN healthscore_dw.psymed_bill_items_master_view bim ON fvbi.bill_item_key = bim.bill_item_key
WHERE fvbi.active_flg = 1;

DROP view if exists healthscore_dw.psymed_hospital_daily_statistics_view;
CREATE VIEW healthscore_dw.psymed_hospital_daily_statistics_view AS
...
join healthscore_dw.psymed_hospital_master_view hm ON fhds.hospital_key = hm.hospital_key;

DROP view if exists healthscore_dw.psymed_active_visits_view;
CREATE VIEW healthscore_dw.psymed_active_visits_view AS
...
join healthscore_dw.psymed_hospital_master_view hm ON fav.hospital_key = hm.hospital_key;

DROP VIEW IF EXISTS healthscore_dw.psymed_patient_diagnosis_view;
CREATE VIEW healthscore_dw.psymed_patient_diagnosis_view AS
...
-- CREATE USER psymed_db_viewer@localhost IDENTIFIED BY <PWD>;
GRANT SELECT ON `healthscore_dw`.`psymed_bill_items_master_view` TO 'psymed_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`psymed_hospital_master_view` TO 'psymed_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`psymed_patient_master_view` TO 'psymed_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`psymed_patient_medications_view` TO 'psymed_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`psymed_patient_assessments_view` TO 'psymed_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`psymed_hospital_staff_master_view` TO 'psymed_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`psymed_patient_visit_view` TO 'psymed_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`psymed_hospital_daily_statistics_view` TO 'psymed_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`psymed_active_visits_view` TO 'psymed_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`psymed_patient_diagnosis_view` TO 'psymed_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`psymed_patient_careplan_instructions_view` TO 'psymed_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`psymed_vist_bill_items_view` TO 'psymed_db_viewer'@'localhost' ;
grant select on  healthscore_dw.psymed_patient_diagnosis_view to psymed_db_viewer@localhost;
