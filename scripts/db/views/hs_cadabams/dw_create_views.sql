DROP VIEW IF EXISTS healthscore_dw.cadabams_hospital_master_view;
CREATE VIEW healthscore_dw.cadabams_hospital_master_view
...
where hospital_cd in ('CAD','CADH','MIN')
;
 

DROP VIEW IF EXISTS healthscore_dw.cadabams_bill_items_master_view;
CREATE VIEW healthscore_dw.cadabams_bill_items_master_view
as select dbi.hospital_key, bill_item_key,bill_item_type,bill_item_category_cd,bill_item_category_nm,bill_item_category_desc,bill_item_cd,bill_item_nm,bill_item_amt,transaction_type_cd,effective_from_ts,effective_to_ts,rate_category_nm,dbi.active_flg
 from healthscore_dw.dim_bill_items dbi
join healthscore_dw.cadabams_hospital_master_view dh
on dbi.hospital_key = dh.hospital_key
;
  
DROP VIEW IF EXISTS healthscore_dw.cadabams_patient_master_view;
CREATE VIEW healthscore_dw.cadabams_patient_master_view as
...
DROP VIEW IF EXISTS healthscore_dw.cadabams_patient_medications_view;
CREATE VIEW healthscore_dw.cadabams_patient_medications_view as
...
join  healthscore_dw.cadabams_patient_master_view pm
...
​
DROP VIEW IF EXISTS healthscore_dw.cadabams_patient_assessments_view;
CREATE VIEW healthscore_dw.cadabams_patient_assessments_view as
...
DROP VIEW IF EXISTS healthscore_dw.cadabams_hospital_staff_master_view;
CREATE VIEW healthscore_dw.cadabams_hospital_staff_master_view as
...
DROP VIEW IF EXISTS healthscore_dw.cadabams_patient_consultant_visit_view;
CREATE VIEW healthscore_dw.cadabams_patient_consultant_visit_view as 
select fpv.patient_key, fpv.visit_hospital_key, fpv.visit_doctor_staff_key,
min(date(fpv.patient_visit_doctor_ts)) as first_consultation_date
from healthscore_dw.map_patient_visit_doctors fpv
join healthscore_dw.cadabams_hospital_master_view hm
on fpv.visit_hospital_key = hm.hospital_key
join healthscore_dw.dim_staff ds 
on fpv.visit_doctor_staff_key = ds.staff_key
where ds.hospital_staff_cd <> 'OR' 
group by fpv.patient_key, fpv.visit_hospital_key, fpv.visit_doctor_staff_key
 ;

DROP VIEW IF EXISTS healthscore_dw.cadabams_patient_visit_view;
CREATE VIEW healthscore_dw.cadabams_patient_visit_view as
...
,(case when vv.first_consultation_date = date(fpv.checkin_ts) then "New" else "Old" end) as first_consultation_flg
...
on fpv.visit_hospital_key = hm.hospital_key
left join  
( select patient_key, min(first_consultation_date) as first_consultation_date
from healthscore_dw.cadabams_patient_consultant_visit_view
group by patient_key
) vv
on vv.patient_key = fpv.patient_key
;

DROP VIEW IF EXISTS healthscore_dw.cadabams_visit_bill_items_view;
CREATE VIEW healthscore_dw.cadabams_visit_bill_items_view AS
...
JOIN healthscore_dw.cadabams_patient_visit_view pvv ON pvv.patient_visit_key = vbv.patient_visit_key
JOIN healthscore_dw.cadabams_bill_items_master_view bim ON fvbi.bill_item_key = bim.bill_item_key
WHERE fvbi.active_flg = 1;
​ 
​
DROP VIEW IF EXISTS healthscore_dw.cadabams_patient_diagnosis_view;
CREATE VIEW healthscore_dw.cadabams_patient_diagnosis_view AS
...
DROP VIEW IF EXISTS healthscore_dw.cadabams_consultant_appt_view;
CREATE VIEW healthscore_dw.cadabams_consultant_appt_view AS
SELECT cas.consultant_schedule_key,cas.hospital_key, cas.consulting_staff_key, cas.schedule_date_key, cas.schedule_start_time_key, cas.schedule_end_time_key ,cas.consultant_schedule_id,
 duration, active_flg,  timeslot_usage_limit,  timeslot_used_cnt , 
 sum(case when  vv.first_consultation_date = date(cas.schedule_date_key) then 1 else 0 end) as new_consult_cnt,
sum(case when fpa.appointment_status='CANCELLED' then 1 else 0 end) as cancelled_consult_cnt,
sum(case when fpa.appointment_status='FULFILLED' then 1 else 0 end) as fulfilled_consult_cnt,
sum(case when fpa.appointment_status='NO SHOW' then 1 else 0 end) as no_show_consult_cnt
...
left join  healthscore_dw.fact_patient_appointments fpa
on fpa.consultant_schedule_key = cas.consultant_schedule_key
left join (select visit_hospital_key,patient_key, min(first_consultation_date) first_consultation_date 
from healthscore_dw.cadabams_patient_consultant_visit_view vv
group by patient_key,visit_hospital_key) vv
on fpa.patient_key = vv.patient_key 
and vv.visit_hospital_key = hm.hospital_key
group by  cas.consultant_schedule_key,cas.hospital_key, cas.consulting_staff_key, cas.schedule_date_key, cas.schedule_start_time_key, cas.schedule_end_time_key ,cas.consultant_schedule_id,
 duration, active_flg,  timeslot_usage_limit,  timeslot_used_cnt
 ;
...
-- CREATE USER cadabams_db_viewer@localhost IDENTIFIED BY <PWD>;
GRANT SELECT ON `healthscore_dw`.`cadabams_bill_items_master_view` TO 'cadabams_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`cadabams_hospital_master_view` TO 'cadabams_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`cadabams_patient_master_view` TO 'cadabams_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`cadabams_patient_medications_view` TO 'cadabams_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`cadabams_patient_assessments_view` TO 'cadabams_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`cadabams_hospital_staff_master_view` TO 'cadabams_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`cadabams_patient_visit_view` TO 'cadabams_db_viewer'@'localhost' ;   
GRANT SELECT ON `healthscore_dw`.`cadabams_patient_diagnosis_view` TO 'cadabams_db_viewer'@'localhost' ;  
GRANT SELECT ON `healthscore_dw`.`cadabams_visit_bill_items_view` TO 'cadabams_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`cadabams_consultant_appt_view` TO 'cadabams_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`cadabams_patient_appt_view` TO 'cadabams_db_viewer'@'localhost' ; 
GRANT SELECT ON `healthscore_dw`.`cadabams_patient_consultant_visit_view` TO 'cadabams_db_viewer'@'localhost' ; 